...
- Added `opengrasp pipeline` (auto-pipeline) and refactored core pipeline into `cli/flows/offer_pipeline.py`.
- Replaced Windows-only tracker with cross-platform Textual TUI (tabs + palette + preview).
- Added `opengrasp doctor` and basic unit tests under `tests/`.
